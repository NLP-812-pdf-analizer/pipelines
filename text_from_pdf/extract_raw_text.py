...
def remove_references(lines):
    # Ищем последовательность: "1." + текст, "2." + текст, ...
    indices = []

    for i in range(len(lines) - 1):
        # если строка с номером
        if re.match(r'^\s*\d+\.\s*$', lines[i]):
            # и следующая строка — текст, начинающийся с заглавной буквы или инициалов
            if re.match(r'^[A-Z(]', lines[i + 1].strip()):
                indices.append(i)

    # Если нашли хотя бы 3 таких ссылок — считаем это списком литературы
    if len(indices) >= 3:
